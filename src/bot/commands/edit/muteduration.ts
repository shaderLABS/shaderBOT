    ...
    callback: async (message, args) => {
        ...
        try {
            if (uuid.test(args[0])) {
                ...
                sendSuccess(
                    channel,
                    `Successfully edited the duration of <@${user_id}'s mute (${args[0]}) to ${time} seconds. They will be unmuted at ${formatTimeDate(
                ...
                sendSuccess(
                    channel,
                    `Successfully edited the duration of <@${user.id}>'s mute (${latestMuteID.id}) to ${time} seconds. They will be unmuted at ${formatTimeDate(
                        new Date(expire_timestamp)
                    )}.`
                );
            }
