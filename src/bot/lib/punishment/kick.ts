...
export class Kick extends Punishment {
    ...
    static async getByUUID(uuid: string) {
        const result = await db.query({ text: /*sql*/ `SELECT * FROM past_punishment WHERE "type" = 'kick' AND id = $1;`, values: [uuid], name: 'kick-uuid' });
    ...
    static async getLatestByUserID(userID: string) {
        const result = await db.query({
            text: /*sql*/ `SELECT * FROM past_punishment WHERE "type" = 'kick' AND user_id = $1 ORDER BY timestamp DESC LIMIT 1;`,
            values: [userID],
    ...
    static async getAllByUserID(userID: string) {
        const result = await db.query({ text: /*sql*/ `SELECT * FROM past_punishment WHERE "type" = 'kick' AND user_id = $1 ORDER BY timestamp DESC;`, values: [userID], name: 'kick-all-user-id' });
    ...
    public async delete(moderatorID: string) {
        const result = await db.query({ text: /*sql*/ `DELETE FROM past_punishment WHERE id = $1;`, values: [this.id], name: 'kick-delete' });
